...
class CatamarcaConnector(BaseConnector):
    slug = "licitaciones-catamarca"
    name = "Provincia de Catamarca"
    base_url = "https://comprar.catamarca.gob.ar"
    page_url = "https://comprar.catamarca.gob.ar/Compras.aspx?qs=W1HXHGHtH10="

    def fetch(self) -> list[RawTenderRecord]:
        assert_public_https_url(self.page_url, label="catamarca.page_url")
    ...
    def _extract_rows(self, html: str) -> list[RawTenderRecord]:
        ...
        table = soup.find("table", id="ctl00_CPH1_GridListaPliegosAperturaProxima")
        if table is None:
            raise ValueError("Catamarca procurement table not found")
        ...
        for row in table.find_all("tr")[1:]:
            ...
            procedure = self._clean_text(cells[2].get_text(" ", strip=True))
            opening_date = self._parse_datetime(cells[3].get_text(" ", strip=True))
            status = self._clean_text(cells[4].get_text(" ", strip=True))
            ...
            items.append(
                RawTenderRecord(
                    ...
                    jurisdiction="Provincia de Catamarca",
                    procedure_type=procedure or None,
        ...
        if not items:
            raise ValueError("Catamarca procurement rows not found")
        return items
